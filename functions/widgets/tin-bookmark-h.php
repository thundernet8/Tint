...
/*
	...
	Copyright: (c) 2014 知言博客 - http://www.zhiyanblog.com
		...
		@version 1.1.8
*/
class tinbookmarkh extends WP_Widget {
/*  Widget
/* ------------------------------------ */
	function __construct(){
		parent::__construct(false,'Tin-通栏友情链接',array( 'description' => 'Tinection-单横栏显示友情链接，放置于通栏小工具区域' ,'classname' => 'widget_tinbookmarkh'));
	}
	...
	?>
		...
		<?php
			...
			echo '<div class="tinbookmarkh"><ul>';
			foreach ( $bookmarks as $bookmark) {
				echo '<li class="tinbookmarkh-list"><a href="'.$bookmark->link_url.'" title="'.$bookmark->link_name.'" target="_blank">'.$bookmark->link_name.'</a></li>';
			}
			echo '</ul></div>';
		?>
...
/*  Register widget
/* ------------------------------------ */
if ( ! function_exists( 'tin_register_widget_bookmarks_h' ) ) {

	function tin_register_widget_bookmarks_h() { 
		register_widget( 'tinbookmarkh' );
	}	
}
add_action( 'widgets_init', 'tin_register_widget_bookmarks_h' );