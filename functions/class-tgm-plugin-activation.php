				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( '忽略提醒', $this->domain ) . '</a>',
						)
					);
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( '外部链接', TGM_Plugin_Activation::$instance->domain );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( '私有版本', TGM_Plugin_Activation::$instance->domain );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( '预打包的', TGM_Plugin_Activation::$instance->domain );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress版本', TGM_Plugin_Activation::$instance->domain );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( '必需', TGM_Plugin_Activation::$instance->domain ) : __( '推荐', TGM_Plugin_Activation::$instance->domain );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( '未安装', TGM_Plugin_Activation::$instance->domain ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( '已安装但未激活', TGM_Plugin_Activation::$instance->domain ) );
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( '插件', TGM_Plugin_Activation::$instance->domain ),
				'source' => __( '来源', TGM_Plugin_Activation::$instance->domain ),
				'type'   => __( '类型', TGM_Plugin_Activation::$instance->domain ),
				'status' => __( '状态', TGM_Plugin_Activation::$instance->domain )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( '安装', TGM_Plugin_Activation::$instance->domain ),
				'tgmpa-bulk-activate' => __( '激活', TGM_Plugin_Activation::$instance->domain ),
			);
